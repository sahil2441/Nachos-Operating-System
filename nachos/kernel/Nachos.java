...
public class Nachos implements Runnable {
	...
	// fileSystem.createDirectory("/" + i + "/" + i);
	// }
	// for (int i = 0; i < 5; i++) {
	// fileSystem.create("File:" + i, 10);
	// }
	// // fileSystem.removeDirectory("/1/1");
	// fileSystem.list();
	...
	if (options.CYCLIC_BARRIER) {
	    nachos.kernel.threads.CyclicBarrier.demo();
	}
	// Added for Question 2
	if (options.TASK_MANAGER) {
	    TaskManager.demo();
	}
	...
	Debug.println('+',
		"Finishing parent thread from run() method in Nachos.java..");
	...
	Debug.println('+', "Entering main in Nachos.java");
