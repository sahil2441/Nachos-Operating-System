 ...
 *
 * There are two implementations. One is a "stub" that directly turns the file
 * operations into the underlying UNIX operations. (cf. comment in
 * FileSystem.java).
 *
 * The other is the "real" implementation, which turns these operations into
 * read and write disk sector requests. In this baseline implementation of the
 * file system, we don't worry about concurrent accesses to the file system by
 * different threads -- this is part of the assignment.
 * 
...
public interface OpenFile {
     ...
     *
     * @param position
     *            The desired position in the file, in bytes from the start of
     *            file.
     */
    ...
    /**
     * Read bytes from the file, starting at the implicit position. The position
    ...
    /**
     * Write bytes to the file, starting at the implicit position. The position
     * is incremented by the number of bytes read.
     *
     * @param from
     *            Buffer containing the data to be written.
    ...
    /**
     * Determine the number of bytes in the file (this interface is simpler than
